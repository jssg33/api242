...
class LicenseTypesController {
  ...
  static async getById(req, res) {
    try {
      ...
      if (!type) return res.status(404).json({ message: "License type not found" });
      res.json(type);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  static async update(req, res) {
    try {
      ...
      if (!updated) return res.status(404).json({ message: "License type not found" });
      res.json(updated);
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
  ...
  static async delete(req, res) {
    try {
      const deleted = await LicenseType.findOneAndDelete({ LicenseTypeID: req.params.id });
      if (!deleted) return res.status(404).json({ message: "License type not found" });
